...
Section LaterSect.
  ...
  ; spec_later_impl : forall P Q,
                        illater (P -->> Q) -|- (illater P) -->> (illater Q)
  ; spec_later_forall : forall {T} (F : T -> A),
                          illater (Forall x:T, F x) -|- Forall x:T, illater (F x)
  ; spec_later_exists {T} (F : T -> A) : Exists x : T, illater (F x) |-- illater (Exists x : T, F x)
  ; spec_later_exists_inhabited : forall {T} (F : T -> A),
                                    inhabited T ->
                                    illater (Exists x : T, F x) |-- Exists x : T, illater (F x)
  }.
End LaterSect.
...
Section ILogicLaterCoreRules.
  ...
  Proof.
    intros P P' HP.
    split; apply (proper_prf (Proper := _: Proper (_ ==> _) _));
  ...
  Proof.
    ...
    split;
    eapply lforallR; intro x; eapply lforallL with x; intros;
  ...
  Proof.
    ...
    { rewrite spec_later_exists_inhabited.
      eapply lexistsL. intros. apply lexistsR with x. destruct x; reflexivity.
      constructor. exact true. }
    { rewrite <- spec_later_exists.
      eapply lexistsL. intros. apply lexistsR with x. destruct x; reflexivity. }
  Qed.
